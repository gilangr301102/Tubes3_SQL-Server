using System;
using System.IO;
using System.Security.Cryptography;
using System.Text;

namespace api.Utils.Helper
{
    public static class AesEncryption
    {
        private static readonly byte[] key = Encoding.UTF8.GetBytes("Yi1@2zXP9#n&zJ7!Fm4qLp3v"); // Must be 16, 24, or 32 bytes for AES
    
        // S-box
        private static readonly byte[] SBox = new byte[256] {
            
        };

        // Inverse S-box
        private static readonly byte[] InvSBox = new byte[256] {
            
        };
        
        // Rcon
        private static readonly byte[] Rcon = new byte[256] {
            
        };

        private const int Nb = 4;
        private const int Nk = 4;
        private const int Nr = 10;

        public static void Main(string[] args)
        {
            // Example usage
            string plaintext = "There's is more than 30 bytes long plaintext jankjdakdna jnakjdnakd";

            string ciphertext = EncryptWithPadding(plaintext);
            
            string plaintextDecrypted = DecryptWithPadding(ciphertext);

            Console.WriteLine("Plaintext: " + plaintext);
            Console.WriteLine("Ciphertext: " + ciphertext);
            Console.WriteLine("Decrypted: " + plaintextDecrypted);
        }

        public static string EncryptWithPadding(string plainText)
        {
            if (plainText == null) throw new ArgumentNullException(nameof(plainText));

            byte[] input = Encoding.UTF8.GetBytes(plainText);

            int numBlocks = (input.Length + 15) / 16;
            int paddedLength = numBlocks * 16;

            byte[] paddedInput = new byte[paddedLength];
            Array.Copy(input, paddedInput, input.Length);

            byte[] ciphertext = new byte[paddedLength];
            for (int i = 0; i < numBlocks; i++)
            {
                byte[] block = new byte[16];
                Array.Copy(paddedInput, i * 16, block, 0, 16);
                byte[] encryptedBlock = Encrypt(block);
                Array.Copy(encryptedBlock, 0, ciphertext, i * 16, 16);
            }

            return Convert.ToBase64String(ciphertext);
        }

        public static string DecryptWithPadding(string cipherText)
        {
            if (cipherText == null) throw new ArgumentNullException(nameof(cipherText));

            byte[] input = Convert.FromBase64String(cipherText);

            int numBlocks = input.Length / 16;
            byte[] plaintext = new byte[input.Length];
            for (int i = 0; i < numBlocks; i++)
            {
                byte[] block = new byte[16];
                Array.Copy(input, i * 16, block, 0, 16);
                byte[] decryptedBlock = Decrypt(block);
                Array.Copy(decryptedBlock, 0, plaintext, i * 16, 16);
            }

            return Encoding.UTF8.GetString(plaintext);
        }

        private static byte[] Encrypt(byte[] input)
        {
            byte[] expandedKey = KeyExpansion();
            byte[] state = new byte[16];
            Array.Copy(input, state, 16);

            AddRoundKey(state, expandedKey, 0);

            for(int round = 1; round < Nr; round++)
            {
                SubBytes(state);
                ShiftRows(state);
                MixColumns(state);
                AddRoundKey(state, expandedKey, round);
            }

            SubBytes(state);
            ShiftRows(state);
            AddRoundKey(state, expandedKey, Nr);

            return state;
        }

        private static byte[] Decrypt(byte[] input)
        {
            byte[] expandedKey = KeyExpansion();
            byte[] state = new byte[16];
            Array.Copy(input, state, 16);

            AddRoundKey(state, expandedKey, Nr);

            for (int round = Nr - 1; round > 0; round--)
            {
                InvShiftRows(state);
                InvSubBytes(state);
                AddRoundKey(state, expandedKey, round);
                InvMixColumns(state);
            }

            InvShiftRows(state);
            InvSubBytes(state);
            AddRoundKey(state, expandedKey, 0);

            return state;
        }

        private static byte[] KeyExpansion()
        {
            byte[] expandedKey = new byte[Nb * (Nr + 1) * 4];
            Array.Copy(key, expandedKey, key.Length);

            for (int i = Nk; i < Nb * (Nr + 1); i++)
            {
                byte[] temp = new byte[4];
                Array.Copy(expandedKey, (i - 1) * 4, temp, 0, 4);

                if (i % Nk == 0)
                {
                    temp = SubWord(RotWord(temp));
                    temp[0] ^= Rcon[i / Nk];
                }

                for (int j = 0; j < 4; j++)
                {
                    expandedKey[i * 4 + j] = (byte)(expandedKey[(i - Nk) * 4 + j] ^ temp[j]);
                }
            }

            return expandedKey;
        }

        private static byte[] SubWord(byte[] word)
        {
            for (int i = 0; i < 4; i++)
            {
                word[i] = SBox[word[i]];
            }
            return word;
        }

        private static byte[] RotWord(byte[] word)
        {
            byte temp = word[0];
            word[0] = word[1];
            word[1] = word[2];
            word[2] = word[3];
            word[3] = temp;
            return word;
        }

        private static void AddRoundKey(byte[] state, byte[] roundKey, int round)
        {
            for (int i = 0; i < 16; i++)
            {
                state[i] ^= roundKey[round * Nb * 4 + i];
            }
        }

        private static void SubBytes(byte[] state)
        {
            for (int i = 0; i < 16; i++)
            {
                state[i] = SBox[state[i]];
            }
        }

        private static void InvSubBytes(byte[] state)
        {
            for (int i = 0; i < 16; i++)
            {
                state[i] = InvSBox[state[i]];
            }
        }

        private static void ShiftRows(byte[] state)
        {
            byte temp;

            // Row 1
            temp = state[1];
            state[1] = state[5];
            state[5] = state[9];
            state[9] = state[13];
            state[13] = temp;

            // Row 2
            temp = state[2];
            state[2] = state[10];
            state[10] = temp;
            temp = state[6];
            state[6] = state[14];
            state[14] = temp;

            // Row 3
            temp = state[3];
            state[3] = state[15];
            state[15] = state[11];
            state[11] = state[7];
            state[7] = temp;
        }

        private static void InvShiftRows(byte[] state)
        {
            byte temp;

            // Row 1
            temp = state[13];
            state[13] = state[9];
            state[9] = state[5];
            state[5] = state[1];
            state[1] = temp;

            // Row 2
            temp = state[2];
            state[2] = state[10];
            state[10] = temp;
            temp = state[6];
            state[6] = state[14];
            state[14] = temp;

            // Row 3
            temp = state[7];
            state[7] = state[11];
            state[11] = state[15];
            state[15] = state[3];
            state[3] = temp;
        }

        private static void MixColumns(byte[] state)
        {
            for (int i = 0; i < 4; i++)
            {
                byte[] column = new byte[4];
                Array.Copy(state, i * 4, column, 0, 4);

                state[i * 4 + 0] = (byte)(gfmultby02(column[0]) ^ gfmultby03(column[1]) ^ column[2] ^ column[3]);
                state[i * 4 + 1] = (byte)(column[0] ^ gfmultby02(column[1]) ^ gfmultby03(column[2]) ^ column[3]);
                state[i * 4 + 2] = (byte)(column[0] ^ column[1] ^ gfmultby02(column[2]) ^ gfmultby03(column[3]));
                state[i * 4 + 3] = (byte)(gfmultby03(column[0]) ^ column[1] ^ column[2] ^ gfmultby02(column[3]));
            }
        }

        private static void InvMixColumns(byte[] state)
        {
            for (int i = 0; i < 4; i++)
            {
                byte[] column = new byte[4];
                Array.Copy(state, i * 4, column, 0, 4);

                state[i * 4 + 0] = (byte)(gfmultby0e(column[0]) ^ gfmultby0b(column[1]) ^ gfmultby0d(column[2]) ^ gfmultby09(column[3]));
                state[i * 4 + 1] = (byte)(gfmultby09(column[0]) ^ gfmultby0e(column[1]) ^ gfmultby0b(column[2]) ^ gfmultby0d(column[3]));
                state[i * 4 + 2] = (byte)(gfmultby0d(column[0]) ^ gfmultby09(column[1]) ^ gfmultby0e(column[2]) ^ gfmultby0b(column[3]));
                state[i * 4 + 3] = (byte)(gfmultby0b(column[0]) ^ gfmultby0d(column[1]) ^ gfmultby09(column[2]) ^ gfmultby0e(column[3]));
            }
        }

        private static byte gfmultby02(byte b)
        {
            return (byte)((b << 1) ^ (((b >> 7) & 1) * 0x1b));
        }

        private static byte gfmultby03(byte b)
        {
            return (byte)(gfmultby02(b) ^ b);
        }

        private static byte gfmultby09(byte b)
        {
            return (byte)(gfmultby02(gfmultby02(gfmultby02(b))) ^ b);
        }

        private static byte gfmultby0b(byte b)
        {
            return (byte)(gfmultby02(gfmultby02(gfmultby02(b))) ^ gfmultby02(b) ^ b);
        }

        private static byte gfmultby0d(byte b)
        {
            return (byte)(gfmultby02(gfmultby02(gfmultby02(b))) ^ gfmultby02(gfmultby02(b)) ^ b);
        }

        private static byte gfmultby0e(byte b)
        {
            return (byte)(gfmultby02(gfmultby02(gfmultby02(b))) ^ gfmultby02(gfmultby02(b)) ^ gfmultby02(b));
        }
    }
}
